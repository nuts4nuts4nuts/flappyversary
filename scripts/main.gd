...
enum SPAWNING_ALGORITHM {PureRandom, DontRepeat, CountUp, One}
var spawn_funcs = {
	...
	SPAWNING_ALGORITHM.One: algo_one,
}
...
func populate_with_new_balls():
	...
	for x in range(ball_count):
		var pos = Vector2(ball_spawner_rng.randi_range(0, rect.x), ball_spawner_rng.randi_range(0, rect.y))
		var direction = (get_viewport_rect().get_center() - pos).normalized()
		var velo = ball_spawner_rng.randf_range(100, 200)
		var warning = ball_spawn_warning_scene.instantiate()
		add_child(warning)
		warning.global_position = pos
		warning.expired.connect(spawn_ball)
		warning.init(delay, generate_new_number(last_generated_number), direction * velo)
...
func algo_one(max_power, previous_number):
	return 1
